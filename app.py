...
def predict():
    ...
    flow_speed = request.args.get("flow_speed", type=float)
    flow_direction = request.args.get("flow_direction", type=str)
    
    if None in [temperature, humidity, flow_speed, flow_direction]:
        return jsonify({"error": "Missing parameters"}), 400
    
    prompt = (f"Given temperature {temperature}°C, humidity {humidity}%, "
             f"flow speed {flow_speed} m/s, and flow direction {flow_direction}, "
             "Your role is climate analyst PHD.goal is to find what is the best oil spill cleaning method? "
             "Please form answer with clear evidences, and examples, chemical formulas and all to cover."
             "Also be more specific and try to give at maximum 2 methods to be concrete.")
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return jsonify(result["candidates"][0]["content"]["parts"][0]["text"])
        else:
            return jsonify(f"Error {response.status_code}: {response.text}"), response.status_code
    
    except Exception as e:
        return jsonify(f"Request failed: {str(e)}"), 500
...
def translate_text():
    ...
    translated_text = call_gemini_translation(text, target_language, GEMINI_URL)
    
    return jsonify({"translatedText": translated_text})

def call_gemini_translation(text, target_language, gemini_url):
    ...
    try:
        ...
        if response.status_code == 200:
            result = response.json()
            # Access the response text correctly based on Gemini's API structure
